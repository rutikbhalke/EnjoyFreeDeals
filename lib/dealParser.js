...
async function parseDealText(payload) {
  ...
  const platformExpiresAt = extractPlatformExpiry(message);
  const productTitle = normalizeProductTitle(extractedTitle, platform, dealPrice);
  const category = normalizeCategoryName(detectCategory(productTitle, message, productUrl || platform));
...
function validateDeal(deal) {
  ...
  if (!hasPriceSignal && !hasOfferSignal && !deal.productUrl) return "No price/coupon/free offer found";
  if (deal.dealPrice < 0 || deal.originalPrice < 0) return "Price cannot be negative";
  return "";
}

function extractPlatformExpiry(text) {
  const value = String(text || "");
  const now = new Date();
  const lower = value.toLowerCase();

  if (/\bends?\s+(?:today|tonight)\b|\bvalid\s+(?:till|until)\s+today\b/.test(lower)) {
    return endOfLocalDay(now).toISOString();
  }

  if (/\bends?\s+tomorrow\b|\bvalid\s+(?:till|until)\s+tomorrow\b/.test(lower)) {
    const tomorrow = new Date(now);
    tomorrow.setDate(tomorrow.getDate() + 1);
    return endOfLocalDay(tomorrow).toISOString();
  }

  const daysMatch = lower.match(/\b(?:ends?|expires?|sale ends?)\s+in\s+([1-9][0-9]?)\s+days?\b/);
  if (daysMatch) {
  ...
  const numericDate = lower.match(/\b(?:valid\s+(?:till|until)|offer\s+ends\s+on|ends?\s+on|expires?\s+on)\s+([0-3]?\d)[/-]([0-1]?\d)(?:[/-](20\d{2}))?\b/);
  if (numericDate) {
    const day = Number(numericDate[1]);
    const month = Number(numericDate[2]) - 1;
    const year = Number(numericDate[3] || now.getFullYear());
    const date = new Date(year, month, day);
    return Number.isNaN(date.getTime()) ? null : endOfLocalDay(date).toISOString();
  }

  const monthDate = lower.match(/\b(?:valid\s+(?:till|until)|offer\s+ends\s+on|ends?\s+on|expires?\s+on)\s+([0-3]?\d)\s+(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)(?:\s+(20\d{2}))?\b/i);
  if (monthDate) {
    const month = monthIndex(monthDate[2]);
    const year = Number(monthDate[3] || now.getFullYear());
    const date = new Date(year, month, Number(monthDate[1]));
    return month < 0 || Number.isNaN(date.getTime()) ? null : endOfLocalDay(date).toISOString();
  }
